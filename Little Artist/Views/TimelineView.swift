...
//  Little Artist
//
//  Chronological artwork feed with vertical timeline, profile hero,
//  milestones carousel, and time-bucketed artwork groups.
//
...
///
/// Displays a profile hero section for the selected child, a milestones
/// carousel, and a vertical timeline of artworks grouped into time buckets
/// (Today, This Week, This Month, then by calendar month).
struct TimelineView: View {
    ...
    private var selectedChild: Child? {
        guard let selectedChildID else { return children.first }
        return children.first { $0.persistentModelID == selectedChildID }
    }

    private var filteredArtworks: [Artwork] {
        guard let selectedChild else { return allArtworks }
        return allArtworks.filter { $0.child?.persistentModelID == selectedChild.persistentModelID }
    }

    private var artworkCount: Int { filteredArtworks.count }

    private var level: Int { artworkCount / 10 + 1 }

    private var artistSinceYear: String {
        guard let child = selectedChild else { return "" }
        return child.createdAt.formatted(.dateTime.year())
    }
    ...
    private var groupedArtworks: [(title: String, subtitle: String, artworks: [Artwork])] {
        ...
        var remaining = filteredArtworks
    ...
    var body: some View {
                ...
                VStack(spacing: Brand.sectionSpacing) {
                    ...
                    if !children.isEmpty {
                        ChildSliderView(
                            ...
                            onEditChild: { child in editingChild = child }
                        )
                    }
        ...
        .onAppear {
            if selectedChildID == nil, let first = children.first {
                selectedChildID = first.persistentModelID
            }
        }
    }

    // MARK: - Child Slider Binding

    private var selectedChildBinding: Binding<Child?> {
        Binding(
            get: { selectedChild },
            set: { newChild in
                selectedChildID = newChild?.persistentModelID
            }
        )
    }
